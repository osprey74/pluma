...
**作成日：** 2026-03-06  
**担当者：** Sohshi (osprey74)  
**ステータス：** 設計完了 → Claude Code 実装フェーズへ引き継ぎ
...
│   │   │   └── index.ts
│   │   ├── SearchPanel/
│   │   │   └── SearchPanel.tsx     # 正規表現検索UIパネル
│   │   └── StatusBar/
│   │       └── StatusBar.tsx       # エンコーディング/行列情報
│   ├── hooks/
│   │   ├── useFileIO.ts            # Tauriコマンド呼び出し
│   │   └── useEncoding.ts          # エンコーディング状態管理
│   ├── stores/
│   │   └── editorStore.ts          # Zustandグローバル状態
│   ├── utils/
...
### Week 1：基盤構築
- [ ] Tauri v2 プロジェクト初期化（`create-tauri-app`）
- [ ] CodeMirror 6 統合（基本エクステンション）
- [ ] ファイル開く / 保存（txt / md）
- [ ] ステータスバー基本UI

### Week 2：コア機能実装
- [ ] 矩形選択（`@codemirror/rectangular-selection` 有効化）
- [ ] 正規表現 検索 / 置換（`@codemirror/search` 有効化）
- [ ] エンコーディング検出・表示（encoding_rs + chardetng）
- [ ] CSV/TSV モード（区切り検出・Tab移動・列ハイライト）
- [ ] ファイルサイズ判定・警告ダイアログ
